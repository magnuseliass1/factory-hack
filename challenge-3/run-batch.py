...
async def run_maintenance_scheduler_batch():
    ...
    for idx, wo in enumerate(WORK_ORDERS, 1):
        ...
        success, output = await run_agent("maintenance_scheduler.py", wo)
...
async def run_parts_ordering_batch():
    ...
    for idx, wo in enumerate(WORK_ORDERS, 1):
        ...
        success, output = await run_agent("parts_ordering.py", wo)
        results.append((wo, success))
...
async def main():
    ...
    print(f"   This will generate traces visible in Azure AI Foundry portal")
    print()
